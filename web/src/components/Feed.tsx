...
function FeedEntry({ item, agentName, onAnswer }: { readonly item: FeedItem } & Omit<FeedProps, 'items'>) {
    switch (item.kind) {
        case 'message':
            return (
...
function Feed({ items, agentName, onAnswer }: FeedProps) {
    ...
    return (
        ...
        >
            ...
            {items.map((item) => <FeedEntry key={item.key} item={item} agentName={agentName} onAnswer={onAnswer} />)}
        </div>
    );
}
export { Feed };
