...
const en = {
    ...
    agent: {
        ...
        chat: 'Chat',
        memory: 'Memory',
        memoryOn: (policy: number) => `memory on · policy v${policy}`,
        memoryOnHint: 'flotti gave the agent the memory tools, the memory policy and the flotti-memory skill.',
        memorySkillUser: 'The skill flotti-memory in skills/ is the agent\'s own: the built-in one is not installed over it.',
        memorySkillMissing: 'The built-in skill flotti-memory could not be written to skills/.',
        memoryUnsupported: 'memory unsupported',
