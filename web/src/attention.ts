/**
 * Which agents wait for a person — a permission to give or a question to
 * answer — and what the page says about it: the title and the notifications.
...
function waitingAgents(agents: readonly AgentSummary[], feeds: Readonly<Record<string, AgentFeed>>): AgentSummary[] {
    return agents.filter((agent) => (feeds[agent.id]?.status ?? agent.status) === 'waiting');
}
...
function notificationText(agent: AgentSummary, feed: AgentFeed | undefined): { readonly title: string; readonly body: string } {
    const asked = [...feed?.items ?? []].reverse().find((item) => item.kind === 'permission' && !item.settled);
    const why = asked?.kind === 'permission' ? `Asks for permission: ${asked.title}` : feed?.reason;
